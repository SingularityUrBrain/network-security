...
def permute(data, bits, mapper):
    ...
    for i, v in enumerate(mapper):
        # check 1 bit in bits-1-v position (v indeed)
        if data & (1 << bits - 1 - v):
            ret |= 1 << len(mapper) - 1 - i  # set 1 to this position
    return ret
...
def feistel(block, key):
    ...
    for i, box in enumerate(SUBSTITUTION_BOX):
        b6 = (block >> (42 - i*6)) & 0x3f  # form 6 bit block, 0x3f = b111111
        # 1bit + 6 bit + 4 bit[2:6] = num(1bit,6bit)*8 + num(2bit,3bit,4bit,5bit)
        ret = ret << 4 | box[b6 & 0x20 | (b6 & 0x01) << 4 | (b6 & 0x1e) >> 1]
...
def encode_block(block, derived_keys, encryption):
    ...
    block = block >> 32, block & 0xffffffff  # left and rigth part
    if not encryption:
